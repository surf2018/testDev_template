...
def save(request):
    ...
    print("status:" + str(taskStat))
    print("result:" + str(taskResult))
    # check taskname是否存在
...
def queryReportResult(request):
    ...
    # 查询report name and result
    try:
        report = TaskResult.objects.get(id=int(reportid))
        re = report.result
        flag = 1
        print("queryReportResult:" + str(re))
    except Exception as e:
        print(e)
        flag = 0
    if (flag == 1):
        return response_succeess(re)
    else:
